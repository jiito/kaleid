...
RayTracer.prototype.draw = function () {
  ...
  for (let j = 0; j < ny; j++) {
    // console.log("Rows remaining: ", (ny - j))
    for (let i = 0; i < nx; i++) {
      // compute pixel color
...
RayTracer.prototype.background = function (ray) {
  ...
  if (this.sky === "white") {
    ...
    let color = vec3.create();
    vec3.lerp(color, vec3.fromValues(0.5, 0.7, 1), vec3.fromValues(1, 1, 1), t);
    return color;
  } else alert("unknown sky ", this.sky);
};
...
RayTracer.prototype.color = function (ray, depth) {
  ...
  if (hit.hit_obj.material) {
    ...
    if (scatter_ray) {
      ...
      vec3.scaleAndAdd(
        color,
        vec3.scale(vec3.create(), color, 0.5),
        scatter_color,
        0.5
      );
    }
...
RayTracer.prototype.hit = function (ray) {
  ...
  if (this.doBVH) {
    let rootHit = this.rootBVHNode.hit(ray, 0.001, 1e20);
    return rootHit;
  } else {
    let minIntersection;
    for (const obj of this.objects) {
      let intersection = obj.hit(ray, 0.001, 1e20);
      if (intersection) {
        if (
          !minIntersection ||
          intersection.param_t < minIntersection.param_t
        ) {
